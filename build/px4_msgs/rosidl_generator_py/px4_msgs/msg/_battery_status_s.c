...
bool px4_msgs__msg__battery_status__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // cell_count
    ...
    assert(PyLong_Check(field));
    ros_message->cell_count = (int32_t)PyLong_AsLong(field);
    Py_DECREF(field);
  }
  {  // source
  ...
  {  // run_time_to_empty
    PyObject * field = PyObject_GetAttrString(_pymsg, "run_time_to_empty");
    if (!field) {
      return false;
    }
    assert(PyLong_Check(field));
    ros_message->run_time_to_empty = (uint16_t)PyLong_AsUnsignedLong(field);
    Py_DECREF(field);
  }
...
PyObject * px4_msgs__msg__battery_status__convert_to_py(void * raw_ros_message)
{
  ...
  {  // cell_count
    PyObject * field = NULL;
    field = PyLong_FromLong(ros_message->cell_count);
    {
  ...
  {  // run_time_to_empty
    PyObject * field = NULL;
    field = PyLong_FromUnsignedLong(ros_message->run_time_to_empty);
    {
      int rc = PyObject_SetAttrString(_pymessage, "run_time_to_empty", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
