...
bool px4_msgs__msg__vehicle_land_detected__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // alt_max
    PyObject * field = PyObject_GetAttrString(_pymsg, "alt_max");
    if (!field) {
      return false;
    }
    assert(PyFloat_Check(field));
    ros_message->alt_max = (float)PyFloat_AS_DOUBLE(field);
    Py_DECREF(field);
  }
  {  // freefall
...
PyObject * px4_msgs__msg__vehicle_land_detected__convert_to_py(void * raw_ros_message)
{
  ...
  {  // alt_max
    PyObject * field = NULL;
    field = PyFloat_FromDouble(ros_message->alt_max);
    {
      int rc = PyObject_SetAttrString(_pymessage, "alt_max", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
