  ...
  px4_msgs__msg__VehicleConstraints * output)
{
  ...
  // speed_xy
  output->speed_xy = input->speed_xy;
  // speed_up
  ...
  px4_msgs__msg__VehicleConstraints__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    px4_msgs__msg__VehicleConstraints * data =
      (px4_msgs__msg__VehicleConstraints *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!px4_msgs__msg__VehicleConstraints__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          px4_msgs__msg__VehicleConstraints__fini(&data[i]);
        }
        free(data);
        return false;
      }
