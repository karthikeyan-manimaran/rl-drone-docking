...
class EstimatorStatus(metaclass=Metaclass_EstimatorStatus):
    ...
    def __init__(self, **kwargs):
        ...
        if 'vibe' not in kwargs:
            self.vibe = numpy.zeros(3, dtype=numpy.float32)
        else:
            self.vibe = numpy.array(kwargs.get('vibe'), dtype=numpy.float32)
            assert self.vibe.shape == (3, )
    ...
    def vibe(self, value):
        if isinstance(value, numpy.ndarray):
            assert value.dtype == numpy.float32, \
                "The 'vibe' numpy.ndarray() must have the dtype of 'numpy.float32'"
            assert value.size == 3, \
                "The 'vibe' numpy.ndarray() must have a size of 3"
            self._vibe = value
            return
        if __debug__:
            ...
            assert \
                ((isinstance(value, Sequence) or
                 ...
                 True), \
                "The 'vibe' field must be a set or sequence with length 3 and each value of type 'float'"
        self._vibe = numpy.array(value, dtype=numpy.float32)
